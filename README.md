...
```
If "lionRun" is false at the time of collision then the game resets and the player loses a life. However, for when "lionRun" is true at the time of collision, I needed write code to work out which lion is involved and only send that lion back to the holding place. As I have an array of lions, it was possible to individually check whether any of the lions are occupying the same square as the player. 
